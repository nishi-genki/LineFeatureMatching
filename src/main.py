...
def load_config(config_path: str) -> dict:
    ...
    """
        ...
        "io": {
            "input": "video/16.mp4",
            "output": "output/lsd_parameter.mp4",
            "csv": "",
            "fallback_fps": 30
        },
        "detection": {
            ...
            "edlines": {
                "min_length": 30,
                "LineFitErrorThreshold": 2.0,
                "MaxDistanceBetweenTwoLines": 10.0
            }
        },
        "descriptor": {},
        "matching": {
    ...
    defaults = {
        ...
        "detection": {
            ...
            "max_lines": 200,
            "resize_width": 0,
            "lsd": {"octave_scale": 2, "num_octaves": 1, "scale": 0.8, "density_th": 0.7},
            "edlines": {"min_length": 30, "LineFitErrorThreshold": 2.0, "MaxDistanceBetweenTwoLines": 10.0},
        },
        "descriptor": {},
...
def process_video(cfg: dict):
    ...
    if proj_cfg.get("enable", False):
        from projector import LineProjector, load_lines3d_csv
        ...
        if projector and pose_estimator:
            ...
            print(f"[INFO] 線分マッチング (LineMatcher2D3D) を初期化: "
    ...
    while True:
        ...
        if pose_estimator is not None:
            ...
            # Step 2: 線分マッチング + GOPPnPL-equivalent で精密化
            if pose_curr is not None and line_matcher is not None:
                R_init, t_init = pose_curr
                proj_init = projector.project_lines(R_init, t_init)
                corr = line_matcher.match(proj_init, curr_kl)
                if len(corr) >= min_line_corr:
                    try:
                        refined = estimate_from_lines(R_init, corr, K_mat,
                                                      min_lines=min_line_corr)
                        if refined is not None:
                            pose_curr = refined
                    except Exception as e:
                        pass  # 最適化失敗時はマーカー姿勢をそのまま使用
